###main function
Optimum.KernelC <- function(inputdata, groupid, nhavepi, givenpi, givenpiT, niter, ninteg,tol, sg0 = 0.5^2, mu0= 0.0, integral_option=0, nthread=-1){
...
#                 }
                ...
                return(list(obj_val = obj[niter1], pi = outcome1, decovExprT = outcome2, decovExprN1 = outcome21, decovExprN2 = outcome22, decovMu = as.matrix(outcome3[,1:niter1]), decovSigma = as.matrix(outcome4[,1:niter1]), pi1 = as.matrix(outcome5[,1:niter1]), pi2= as.matrix(outcome6[,1:niter1])))
}

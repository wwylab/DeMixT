DeMixT_S2 <- function(
    ...
    res <- Optimum_KernelC(inputdata, groupid, nhavepi = 1, givenpi = givenpi, 
                            givenpiT = rep(0, ncol(data.Y)), niter = 1, 
                            ninteg = nbin, tol = 1e-5, nthread = nthread)
    ...
    if(is.null(data.comp2)){
        decovMuN <- rowMeans(log2(data.comp1))
        decovSigmaN <- rowSds(log2(data.comp1))
        ...
        return(list(decovExprT = decovExprT[filter.in, ], 
                    decovExprN1 = decovExprN1[filter.in, ], 
                    decovExprN2 = decovExprN2[filter.in, ], 
                    decovMu = decovMu[filter.in, ], 
                    decovSigma = decovSigma[filter.in, ]))
        } else {
        decovMuN1 <- rowMeans(log2(data.comp1))
        decovSigmaN1 <- rowSds(log2(data.comp1))
        decovMuN2 <- rowMeans(log2(data.comp2))
        decovSigmaN2 <- rowSds(log2(data.comp2))
